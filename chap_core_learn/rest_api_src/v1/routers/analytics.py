...
from ...data_models import BackTestCreate, DatasetMakeRequest, JobResponse
...
# Define a type alias for constrained float
QuantileFloat = confloat(ge=0, le=1)


class EvaluationEntryRequest(BaseModel):
    backtest_id: int
    quantiles: List[QuantileFloat]
...
async def get_evaluation_entries(
    ...
    return [
        ...
        for forecast in backtest.forecasts
        for q in quantiles
    ]
